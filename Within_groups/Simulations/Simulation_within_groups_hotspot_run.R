...
#  for (FC in c(1.2, 1.3, 1.4, 1.5, 1.6)){
    ...
    readcounts <- as.matrix(read.table(file=paste0("../simulation/hotspot/Cell_prop_",cellProp,"/count_hotspot_200_FC", FC, ".txt")))
    info <- read.table(file=paste0("../simulation/hotspot/Cell_prop_",cellProp,"/info_hotspot_200_FC", FC, ".txt"))
    ...
    for (Perm_i in c(0:1000)){
    	...
    	hotspot_SPARK1 <- data.frame(cellProp=cellProp, FC=FC, method="SPARK",
   	...
   	pred <- factor(as.numeric(res_all_SPADE$Adjust.Pvalue < 0.05))
    	...
    	hotspot_SPADE1 <- data.frame(cellProp=cellProp, FC=FC, method="SPADE",
   	...
   	hotspot_MERINGUE1 <- data.frame(cellProp=cellProp, FC=FC, method="MERINGUE", 
    	...
    	hotspot_DESpace_BayesSpace <- data.frame(cellProp=cellProp, FC=FC, method="DESpace_BayesSpace",
    ...
    save(SPARK_list, file=paste0("../simulation/hotspot/Cell_prop_",cellProp,"/FC",FC,"/Perm_SPARK_hotspot.RData"))
    save(SPADE_list, file=paste0("../simulation/hotspot/Cell_prop_",cellProp,"/FC",FC,"/Perm_SPADE_hotspot.RData"))
    save(MERINGUE_list, file=paste0("../simulation/hotspot/Cell_prop_",cellProp,"/FC",FC,"/Perm_MERINGUE_hotspot.RData"))
    save(DESpace_BayesSpace_list, file=paste0("../simulation/hotspot/Cell_prop_",cellProp,"/FC",FC,"/Perm_DESpace_BayesSpace_hotspot.RData"))

    #Eval.final <- rbind(Eval.final, hotspot_SPADE1, hotspot_SPARK1, hotspot_MERINGUE1, hotspot_DESpace_BayesSpace)
