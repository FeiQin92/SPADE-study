...
for FC in ["1.2", "1.3", "1.4", "1.5", "1.6"]:
    for Cellprop in ["0.1", "0.2", "0.3"]:
        for shape in ["hotspot", "streak"]:
            ...
            spatial=pd.read_csv(paste0("/gpfs/gsfs12/users/qinf2/GuoshuaiCai/SPADE/simulation/", shape, "/Cell_prop_", Cellprop, "/info_", shape, "_200_FC", FC, "_Perm1000.txt"),sep="\t",header=None, na_filter=False, index_col=0)

            for Perm_i in range(0, 1001): 
                print(paste0("Perm_i ############ ", Perm_i))
	
                adata=pd.read_csv(paste0("/gpfs/gsfs12/users/qinf2/GuoshuaiCai/SPADE/simulation/", shape, "/Cell_prop_", Cellprop, "/count_", shape, "_200_FC", FC, ".spa.txt"),sep=" ",header=0, na_filter=False, index_col=0)
                ...
                adj=spg.calculate_adj_matrix(x=adata.obs["x_array"],y=adata.obs["y_array"], histology=False)
                ...
                outpath=paste0("/gpfs/gsfs12/users/qinf2/GuoshuaiCai/SPADE/simulation/", shape, "/Cell_prop_",Cellprop,"/FC", FC, "/SpaGCN/SpaGCN_", shape, "_FC", FC, "_Perm_", Perm_i, ".txt")
                de_genes_info.to_csv(outpath, sep='\t', index=False)

            # Save the list of adata objects as a .RData file
            # robjects.r['save'](r_adata_list, file=paste0("/gpfs/gsfs12/users/qinf2/GuoshuaiCai/SPADE/simulation/", shape, "/Cell_prop_",Cellprop,"/FC", FC, "/SpaGCN_", shape, "_FC",FC,".RData"))
