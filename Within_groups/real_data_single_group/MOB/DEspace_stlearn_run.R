...
#Perm_i_min=as.numeric(args[1])*100
#Perm_i_max=(as.numeric(args[1])+1)*100
Perm_i_min=0
Perm_i_max=1000
...
for (Perm_i in Perm_i_min:Perm_i_max){
    ...
    stLearn_results <- read.csv(paste0("/gpfs/gsfs12/users/qinf2/GuoshuaiCai/SPADE/real_data_single_group/MOB/stlearn/stlearn_clusters_MOB_Perm_", Perm_i, ".txt"), row.names=1)
    ...
    stLearn_results1 <- stLearn_results[match(rownames(colData(sce)), 
                                      paste0("X", rownames(stLearn_results))), ]
    ...
    save(DESpace_stlearn.res, file=paste0("/gpfs/gsfs12/users/qinf2/GuoshuaiCai/SPADE/real_data_single_group/MOB/DESpace_stlearn/Perm_", Perm_i,"_DESpace_stlearn_MOB.RData"))
