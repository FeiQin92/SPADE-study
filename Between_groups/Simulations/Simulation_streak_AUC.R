...
# params <- splatEstimate(readcounts)
# 
...
for (cellProp in c(0.1, 0.2, 0.3)){
  for (ieffect in 0.5*c(3:8)){
    ...
    pp = add_markdist_streak(pp,low_marks=low_expr, high_marks=high_expr, nMarkers=(lowSignal+highSignal), ncell=numCell, cell_proportion=cellProp)
    ...
    write.table(pp$markformat,file=paste0("E:\\DB\\Dropbox\\Qinfei\\Research\\Spatial scRNA\\simulation\\DEanalysis\\streak\\Cell_prop_",cellProp,"\\count_streak_200_FC_hr_indexT", as.character(ieffect), ".txt"),row.names = F,col.names = F,quote = F,sep="\t")
...
for (cellProp in c(0.1, 0.2, 0.3)){
  for (FC in 0.5*c(3:8)){
    ...
    save(res, file=paste0("E:\\DB\\Dropbox\\Qinfei\\Research\\Spatial scRNA\\simulation\\DEanalysis\\streak\\Cell_prop_",cellProp,"\\SPDE_estimate_hr_",cellProp,"_", FC, "EX_ShapeStrength.RData" ))
...
for (cellProp in c(0.1, 0.2, 0.3)){
  for (FC in 0.5*c(3:8)){
    ...
    res_DE <- get(load(paste0("E:\\DB\\Dropbox\\Qinfei\\Research\\Spatial scRNA\\simulation\\DEanalysis\\streak\\Cell_prop_",cellProp,"\\Other_DE_methods_",cellProp,"_", FC, ".RData" )))
    AUC.other <- AUC.gen(res=res_DE, truth=truth, nDE=50)
    
    SPDE_estimate_hr <- get(load(paste0("E:\\DB\\Dropbox\\Qinfei\\Research\\Spatial scRNA\\simulation\\DEanalysis\\streak\\Cell_prop_",cellProp,"\\SPDE_estimate_hr_",cellProp,"_", FC, "EX_ShapeStrength.RData" )))
